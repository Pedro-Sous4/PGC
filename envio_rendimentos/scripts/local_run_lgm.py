...
# setup Django environment (append the inner package dir where settings.py lives)
sys.path.append(r'C:\PGC\envio_rendimentos\envio_rendimentos')
# also include the parent folder so Django apps (like core) are importable
...
print('Inicializando progresso e disparando processador localmente...')
request_id = init_progress()
log_progress(request_id, '✅ Teste local: iniciando processamento (thread)')
...
try:
    ...
    print('Erro ao serializar status final:', e)
